-- =============================================================
-- 00006_pos_functions.sql
-- Server-side sale recording & voiding (SECURITY DEFINER).
-- All sale writes go through these functions so that:
--   * cashier_id always matches the authenticated user
--   * inventory decrement always goes through inventory_transactions
--   * void/refund role gate (void_threshold) is enforced server-side
--   * client-generated UUIDs make calls idempotent (retry-safe,
--     reused later by the offline sync engine)
-- =============================================================

alter table public.sales
  add column if not exists void_reason text;

-- ------------------------------------------------------------------
-- record_sale(p_sale, p_items, p_payments) -> { id, created }
-- Inserts the sale + items + payments + inventory 'sale' transactions
-- atomically. If the sale id already exists, does nothing (returns
-- created=false) so retries are safe.
-- ------------------------------------------------------------------
create or replace function public.record_sale(
  p_sale jsonb,
  p_items jsonb,
  p_payments jsonb
) returns jsonb
language plpgsql
security definer
set search_path = public
as $$
declare
  v_sale_id uuid;
  v_cashier_id uuid := auth.uid();
  v_item jsonb;
  v_payment jsonb;
  v_offline boolean;
  v_is_service boolean := coalesce(auth.jwt()::jsonb->>'role', '') = 'service_role';
begin
  if v_cashier_id is null and not v_is_service then
    raise exception 'Not authenticated';
  end if;
  if jsonb_array_length(p_items) = 0 then
    raise exception 'Sale has no items';
  end if;

  v_sale_id := (p_sale->>'id')::uuid;
  if v_sale_id is null then
    raise exception 'Sale id is required (client-generated UUID)';
  end if;

  -- cashier must be the acting user (RLS would normally enforce this;
  -- SECURITY DEFINER re-checks it explicitly). Service-role callers
  -- (offline sync / API routes, which authorize the caller themselves)
  -- are trusted with the payload's cashier_id.
  if (p_sale->>'cashier_id')::uuid is distinct from v_cashier_id and not v_is_service then
    raise exception 'cashier_id does not match the authenticated user';
  end if;

  -- resolved cashier (payload for service role, auth.uid() for end users)
  v_cashier_id := (p_sale->>'cashier_id')::uuid;
  if v_cashier_id is null then
    raise exception 'cashier_id is required';
  end if;

  -- idempotency: already recorded?
  if exists (select 1 from public.sales where id = v_sale_id) then
    return jsonb_build_object('id', v_sale_id, 'created', false);
  end if;

  v_offline := coalesce((p_sale->>'created_offline')::boolean, false);

  insert into public.sales
    (id, customer_id, cashier_id, sale_date, subtotal, discount_total,
     tax_total, grand_total, status, created_offline, synced_at)
  values (
    v_sale_id,
    (p_sale->>'customer_id')::uuid,
    v_cashier_id,
    coalesce((p_sale->>'sale_date')::timestamptz, now()),
    (p_sale->>'subtotal')::numeric,
    coalesce((p_sale->>'discount_total')::numeric, 0),
    coalesce((p_sale->>'tax_total')::numeric, 0),
    (p_sale->>'grand_total')::numeric,
    'completed',
    v_offline,
    case when v_offline then null else now() end
  );

  for v_item in select * from jsonb_array_elements(p_items) loop
    if (v_item->>'quantity')::integer <= 0 then
      raise exception 'Invalid quantity for variant %', v_item->>'variant_id';
    end if;
    insert into public.sale_items
      (id, sale_id, variant_id, quantity, unit_price, line_discount, line_total)
    values (
      (v_item->>'id')::uuid,
      v_sale_id,
      (v_item->>'variant_id')::uuid,
      (v_item->>'quantity')::integer,
      (v_item->>'unit_price')::numeric,
      coalesce((v_item->>'line_discount')::numeric, 0),
      (v_item->>'line_total')::numeric
    );

    -- audit trail: the apply_inventory_transaction trigger adjusts stock
    insert into public.inventory_transactions (variant_id, type, quantity, reference_id)
    values ((v_item->>'variant_id')::uuid, 'sale', (v_item->>'quantity')::integer, v_sale_id);
  end loop;

  for v_payment in select * from jsonb_array_elements(p_payments) loop
    insert into public.payments (id, sale_id, method, amount, transaction_ref, status)
    values (
      (v_payment->>'id')::uuid,
      v_sale_id,
      (v_payment->>'method')::text,
      (v_payment->>'amount')::numeric,
      nullif(v_payment->>'transaction_ref', ''),
      coalesce((v_payment->>'status')::text, 'success')
    );
  end loop;

  return jsonb_build_object('id', v_sale_id, 'created', true);
end;
$$;

-- ------------------------------------------------------------------
-- void_sale(p_sale_id, p_status, p_reason) -> { id, status }
-- Role gate (server-side, not just UI):
--   manager/admin: any sale
--   cashier: only their own sale, and only when grand_total <= void_threshold
-- Restores stock via inventory_transactions type 'return'.
-- ------------------------------------------------------------------
create or replace function public.void_sale(
  p_sale_id uuid,
  p_status text default 'void',
  p_reason text default null
) returns jsonb
language plpgsql
security definer
set search_path = public
as $$
declare
  v_cashier_id uuid := auth.uid();
  v_role text;
  v_threshold numeric(10,2) := 0;
  v_is_service boolean := coalesce(auth.jwt()::jsonb->>'role', '') = 'service_role';
  v_sale record;
  v_item record;
begin
  if v_cashier_id is null and not v_is_service then
    raise exception 'Not authenticated';
  end if;

  select * into v_sale from public.sales where id = p_sale_id;
  if not found then
    raise exception 'Sale not found';
  end if;
  if v_sale.status <> 'completed' then
    raise exception 'Sale is already %', v_sale.status;
  end if;

  -- The approval gate applies to end users; service-role callers (API
  -- routes) authorize the requester themselves before calling.
  if not v_is_service then
    v_role := public.current_role();
    select coalesce(value::numeric, 0) into v_threshold
    from public.app_settings where key = 'void_threshold';

    if v_role not in ('admin', 'manager') then
      if v_sale.cashier_id is distinct from v_cashier_id then
        raise exception 'You can only void your own sales';
      end if;
      if v_sale.grand_total > v_threshold then
        raise exception 'Void requires manager approval (amount above threshold %)', v_threshold;
      end if;
    end if;
  end if;

  update public.sales
    set status = p_status, void_reason = nullif(p_reason, '')
  where id = p_sale_id;

  -- restore stock through the audit trail
  for v_item in
    select si.variant_id, si.quantity
    from public.sale_items si
    where si.sale_id = p_sale_id
  loop
    insert into public.inventory_transactions (variant_id, type, quantity, reference_id)
    values (v_item.variant_id, 'return', v_item.quantity, p_sale_id);
  end loop;

  return jsonb_build_object('id', p_sale_id, 'status', p_status);
end;
$$;

-- ------------------------------------------------------------------
-- Permissions: only allow calling through the authenticated PostgREST
-- role; both functions still verify auth.uid() themselves.
-- ------------------------------------------------------------------
revoke all on function public.record_sale(jsonb, jsonb, jsonb) from public;
grant execute on function public.record_sale(jsonb, jsonb, jsonb) to authenticated;

revoke all on function public.void_sale(uuid, text, text) from public;
grant execute on function public.void_sale(uuid, text, text) to authenticated;

-- ------------------------------------------------------------------
-- Settings
-- ------------------------------------------------------------------
insert into public.app_settings (key, value) values
  ('tax_rate', '0') -- percent; manager-adjustable later
on conflict (key) do nothing;
